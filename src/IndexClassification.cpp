...
const RealMatrixType& IndexClassification::getHoppingMatrix() const
{
  return HoppingMatrix;
}

const std::vector<SingleIndex*>& IndexClassification::getSingleIndexList() const
{
...
bool IndexClassification::checkIndex(ParticleIndex in) const
{
    return (in <= N_bit-1);
}

const TermsList& IndexClassification::getTermsList() const
{
    return Terms;
}
...
const std::list<Term*>& TermsList::getTerms(unsigned short order) const
{
    return TermsMap.find(order)->second;
}
