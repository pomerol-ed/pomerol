...
private:
    ...
    const IndexHamiltonian<Complex> &Storage;
    ...
    std::vector<std::shared_ptr<Operator<Complex>>> Operations;
...
public:
    Symmetrizer(const IndexClassification<Complex> &IndexInfo, const IndexHamiltonian<Complex> &Storage);
    ...
    void compute(const std::vector<Operator<Complex>>& integrals_of_motion);

    bool checkSymmetry(const Operator<Complex> &in);
    /** Get a vector of operators that commute with the Hamiltonian. */
    const std::vector<std::shared_ptr<Operator<Complex>>>& getOperations() const;
...
struct Symmetrizer<Complex>::IndexPermutation
{
private:
...
public:
     ...
     */
    bool set ( int pos, MelemType<Complex> val );
...
template<bool Complex>
using QuantumNumbers = typename Symmetrizer<Complex>::QuantumNumbers;
