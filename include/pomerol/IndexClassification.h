...
private:
    ...
    const typename Lattice<Complex>::SiteMap &Sites;
...
public:
   ...
   // std::list<ParticleIndex>& findIndices(const std::string &SiteLabel);
    ...
    ParticleIndex getIndex(const IndexInfo& ) const;
     ...
     */
    IndexClassification (const typename Lattice<Complex>::SiteMap &Sites);
...
public:
    ...
    friend std::ostream& operator<<(std::ostream& output, const IndexInfo& out);
};
